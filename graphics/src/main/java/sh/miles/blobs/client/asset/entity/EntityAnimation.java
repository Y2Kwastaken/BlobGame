...
public final class EntityAnimation {
    ...
    public Animation<TextureRegion> get(AnimationVariation variation) {
        return variations.get(variation).animation;
    }
    ...
    public static final class Variation {
        ...
        public Variation(AnimationVariation variation, float interval, Animation.PlayMode playMode, Supplier<List<TextureRegion>> regions) {
    ...
    public static final class ParserHelper {
        ...
        public static EntityAnimation parse(SpriteSheet sheet, JsonValue parent) {
            ...
            final Animation.PlayMode playMode = Animation.PlayMode.valueOf(parent.getString("playmode").toUpperCase());
            final Map<AnimationVariation, Variation> variations = new HashMap<>();
            for (final JsonValue variation : parent.get("variations")) {
                parseAnimation(sheet, variation.getString("variation_type"), playMode, variation.get("animation"), false, variations);
            }

            return new EntityAnimation(animation, variations);
        }

        private static void parseAnimation(SpriteSheet sheet, String variation, Animation.PlayMode playMode, JsonValue parent, boolean flip, final Map<AnimationVariation, Variation> collector) {
            if (variation.equals("side")) {
                parseAnimation(sheet, "east", playMode, parent, false, collector);
                parseAnimation(sheet, "west", playMode, parent, true, collector);
                return;
            }
            ...
            collector.put(animationVariation, new Variation(animationVariation, interval, playMode, () -> regions.stream().map(Supplier::get).toList()));
        }
